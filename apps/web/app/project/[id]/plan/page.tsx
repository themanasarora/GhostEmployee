...
export default function ProjectPlanPage() {
  ...
  async function handleContinue() {
    ...
    setSaving(true);
    updateProject(user.uid, id, { plan: selected, hiredRoles: DEFAULT_ROLES });
    setSaving(false);
    if (selected === "basic") router.push(`/project/${id}`);
    else router.push(`/project/${id}/hire`);
  }
    ...
    <div className="min-h-screen bg-[#0A0A14]">
      ...
      <main className="relative z-10 max-w-3xl mx-auto px-6 py-16 space-y-10">
        <div className="text-center space-y-2">
          ...
          <p className="text-slate-400 text-sm">Both free. Basic has 4 default agents. Advanced lets you pick your full team.</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {PLANS.map((plan) => (
            <div key={plan.id} onClick={() => setSelected(plan.id)}
              className={cn("flex flex-col rounded-2xl border p-6 cursor-pointer transition-all duration-200",
                selected === plan.id ? "border-[#E94560] bg-[#E94560]/5" : "border-white/10 bg-white/[0.03] hover:border-white/20")}>
              <div className="flex items-center justify-between mb-4">
                <span className="text-xs font-semibold px-2.5 py-1 rounded-full"
                  style={{ backgroundColor: `${plan.color}20`, color: plan.color }}>{plan.badge}</span>
              ...
              {plan.id === "basic" && <p className="text-xs text-indigo-400 font-medium mb-2">CEO · PM · Research · Growth pre-hired</p>}
              {plan.id === "advanced" && <p className="text-xs text-[#E94560] font-medium mb-2">You pick from all 8 employees</p>}
              <p className="text-sm text-slate-400 mb-4 flex-1">{plan.tagline}</p>
              <ul className="space-y-1.5">
                {plan.features.map((f) => (
                  <li key={f} className="flex items-center gap-2">
                    <CheckCircle2 className="size-3 shrink-0" style={{ color: plan.color }} />
